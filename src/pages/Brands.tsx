    ...
    switch (e.key) {
      ...
      case 'Enter':
        if (activeSuggestionIndex >= 0 && activeSuggestionIndex < filtered.length) {
          ...
          navigate(`/b/${selectedBrand.id}/${selectedBrand.name.replace(/\s+/g, '-').toLowerCase()}`);
        }
        break;
      case 'Escape':
        ...
        <section className="top-brands">
          ...
          <div className="top-brands__brands">
            <div className="box-wrapper grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-8" style={{paddingRight: "0"}}>
              {brands.slice(0, 12).map((brand) => ( <Link className="brand box" alt={brand.name} title={brand.name} key={brand.id} to={`/b/${brand.id}/${brand.name.toLowerCase()}`}><img itemProp="logo" alt={brand.name} title={brand.name} loading="lazy" src={brand.logo} /></Link> ))}
            </div>
          </div>
        </section>
          ...
          <div className="brand-directory__letters">
            <div className="brand-directory__letters-wrapper">
              <nav className="brand-directory__nav">
                <a href="/brands#letter-0-9" style={{ opacity: groupedBrands.numbers && groupedBrands.numbers.length > 0 ? 1 : 0.5, cursor: groupedBrands.numbers && groupedBrands.numbers.length > 0 ? 'pointer' : 'not-allowed' }}>0-9</a>
                {completeAlphabet.map(letter => {
                  const hasBrands = groupedBrands.latin[letter] && groupedBrands.latin[letter].length > 0;
                  return ( <a key={letter} href={`/brands#letter-${letter}`} style={{ opacity: hasBrands ? 1 : 0.5, cursor: hasBrands ? 'pointer' : 'not-allowed' }}>{letter}</a> );
                })}
                <a href="/brands#letter-Greek" style={{ opacity: groupedBrands.greek && groupedBrands.greek.length > 0 ? 1 : 0.5, cursor: groupedBrands.greek && groupedBrands.greek.length > 0 ? 'pointer' : 'not-allowed' }}>Α-Ω</a>
              </nav>
            </div>
            ...
            <div className="brand-directory__letter" id="letter-0-9">
              ...
              <div className="brand-directory__letter-main"><ol>{groupedBrands.numbers?.sort((a, b) => {return parseFloat(a.name) - parseFloat(b.name);}).map((brand) => ( <li key={brand.id}><Link to={`/b/${brand.id}/${brand.name.replace(/\s+/g, '-').toLowerCase()}`} rel="nofollow">{brand.name}</Link></li> ))}</ol></div>
            </div>
            ...
            {Object.keys(groupedBrands.latin || {}).sort().map(letter => (
              <div className="brand-directory__letter" id={`letter-${letter}`} key={letter}>
                ...
                <div className="brand-directory__letter-main"><ol>{groupedBrands.latin[letter]?.sort((a, b) => a.name.localeCompare(b.name)).map((brand) => ( <li key={brand.id}><Link to={`/b/${brand.id}/${brand.name.replace(/\s+/g, '-').toLowerCase()}`} rel="nofollow">{brand.name}</Link></li> ))}</ol></div>
              </div>
            ))}
            ...
            <div className="brand-directory__letter" id="letter-Greek">
              ...
              <div className="brand-directory__letter-main"><ol>{groupedBrands.greek?.sort((a, b) => a.name.localeCompare(b.name)).map((brand) => ( <li key={brand.id}><Link to={`/b/${brand.id}/${brand.name.replace(/\s+/g, '-').toLowerCase()}`} rel="nofollow">{brand.name}</Link></li> ))}</ol></div>
            </div>
          </div>
        </section>
      </div>
