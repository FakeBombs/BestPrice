...
const Categories: React.FC = () => {
  ...
  const findCategory = useCallback((identifier: string): Category | undefined => allCategoriesList.find(cat => cat.id.toString() === identifier || cat.slug === identifier), [allCategoriesList]); // Memoize findCategory
  ...
  const getProductsForSections = useCallback((filterFn: (p: Product) => boolean, sortFn?: (a: Product, b: Product) => number) => { let products = productsFromDescendants.filter(filterFn); if (sortFn) { products = products.sort(sortFn); } return products.slice(0, SLIDER_PRODUCT_COUNT); }, [productsFromDescendants]); // Depend on memoized descendant products
 ...
 const reconcileFilters = useCallback(( filtersFromUrl: ActiveFiltersState, currentAvailableBrands: Record<string, number>, currentAvailableSpecs: Record<string, Set<string>> ): ActiveFiltersState => { const reconciledBrands = filtersFromUrl.brands.map(lb => Object.keys(currentAvailableBrands).find(b => b.toLowerCase() === lb)).filter((b): b is string => !!b); const reconciledSpecs = Object.entries(filtersFromUrl.specs).reduce((acc, [keyFromUrl, lowerValues]) => { const originalKey = Object.keys(currentAvailableSpecs).find(ak => ak.toLowerCase() === keyFromUrl.toLowerCase()); if (originalKey) { const availableValuesSet = currentAvailableSpecs[originalKey]; if (availableValuesSet) { const originalValues = lowerValues.map(lv => Array.from(availableValuesSet).find(av => av.toLowerCase() === lv)).filter((v): v is string => !!v); if (originalValues.length > 0) { acc[originalKey] = originalValues; } } } return acc; }, {} as Record<string, string[]>); return { ...filtersFromUrl, brands: reconciledBrands, specs: reconciledSpecs }; }, []); // No external deps needed here
  ...
  // --- Rendering Functions ---
  // Moved below hook/state declarations
  ...
  const renderSubcategories = (category: Category) => {
    ...
    return (
      <>
        ...
        {/* *** Large Price Alert Button at BOTTOM of Subcategories (Unconditional within this function) *** */}
        <div className="p__products-section">
