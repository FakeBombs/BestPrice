...
const Categories: React.FC = () => {
  ...
  // Boolean flag derived directly from activeFilters for cleaner conditional rendering
  ...
  const getFiltersFromUrl = (currentAvailableSpecs: Record<string, Set<string>>): ActiveFiltersState => {
    ...
    // Ensure only one vendor ID if multiple are in URL (enforce single select)
    const finalVendorIds = vendorIdsFromUrl.length > 1 ? [vendorIdsFromUrl[0]] : vendorIdsFromUrl;
    return {
      brands: brandsFromUrl,
      specs: specsFromUrl,
      vendorIds: finalVendorIds, // Strict single vendor from URL
  ...
  const updateUrlParams = (filters: ActiveFiltersState, currentSortType: string) => {
    ...
    Object.keys(activeFilters.specs).forEach(key => params.delete(key.toLowerCase())); // Delete based on previous state keys before setting new ones
    Array.from(params.keys()).forEach(key => {
      if (!RESERVED_PARAMS_CAT.has(key.toLowerCase()) && !Object.keys(availableSpecs).some(specKey => specKey.toLowerCase() === key.toLowerCase())) params.delete(key);
    });

    if (filters.brands.length > 0) params.set('brand', filters.brands.map(b => b.toLowerCase()).join(','));
    if (filters.vendorIds.length > 0) { // Should be 0 or 1
  ...
  const reconcileFilters = (
    ...
    const reconciledSpecs = Object.entries(filtersFromUrl.specs).reduce((acc, [keyFromUrl, valsFromUrl]) => { // Key might be lower, find original
    ...
    // Vendor IDs already handled strictly in getFiltersFromUrl
    return {
      ...filtersFromUrl, // Includes vendorIds, deals, certified etc. from getFiltersFromUrl
  ...
  useEffect(() => {
    // Run only if category context is set and it's not a main category page
    // or if available options have been populated (covers initial load for main categories too if needed)
    ...
    if (shouldSync) {
      const filtersFromUrl = getFiltersFromUrl(availableSpecs); // Reads URL, enforces single vendor ID
      const reconciledState = reconcileFilters(filtersFromUrl, availableBrands, availableSpecs); // Cleans up brands/specs
      const sortFromUrl = searchParams.get('sort') || DEFAULT_SORT_TYPE;
      let stateChanged = false;

      if (sortFromUrl !== sortType) setSortType(sortFromUrl);

      // Deep compare filters before setting state
      if (JSON.stringify(reconciledState) !== JSON.stringify(activeFilters)) {
        setActiveFilters(reconciledState);
        stateChanged = true;
      }

      // If reconciliation changed the state derived from URL, update URL to match clean state
      if (stateChanged && JSON.stringify(filtersFromUrl) !== JSON.stringify(reconciledState)) {
        updateUrlParams(reconciledState, sortFromUrl);
      }
    }
  // Rerun when searchParams change OR when available filters/category context changes
  ...
  const createToggleHandler = (filterKey: keyof Omit<ActiveFiltersState, 'brands' | 'specs' | 'vendorIds'>) => {
    return () => {
      ...
      setActiveFilters(newFilters); // Update local state first
      updateUrlParams(newFilters, sortType); // Then update URL
    };
  };
  ...
  // Handles removal from "Applied Filters" bar (can remove the single selected vendor)
  const handleMultiVendorToggle = (vendor: Vendor) => {
    const newFilters = { ...activeFilters, vendorIds: [] }; // Always clears vendor selection
    setActiveFilters(newFilters);
    updateUrlParams(newFilters, sortType);
  };
  ...
  const handleSortChange = (newSortType: string) => {
    if (newSortType !== sortType) {
      ...
      updateUrlParams(activeFilters, newSortType); // Pass current filters
    }
  };

  // --- Scroll To Top Effect ---
  const isInitialLoad = useRef(true); // Use ref to track initial load/mount
  useEffect(() => {
    if (isInitialLoad.current) {
      isInitialLoad.current = false; // Mark initial load as complete
      return; // Don't scroll on first render
    }
    // Scroll smoothly on subsequent filter/sort changes
    const timer = setTimeout(() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }, 100);
    return () => clearTimeout(timer);
  }, [activeFilters, sortType]); // Trigger on filter or sort changes
  ...
  const renderBreadcrumbs = () => {
    ...
    if (currentCategory) {
      ...
      while (category?.parentId !== null && category?.parentId !== undefined) {
  ...
  const renderProducts = () => {
    ...
    // Use derived state flags for clarity
    // isSingleVendorSelected and singleSelectedVendorId are calculated above

    // DEBUG LOG: Check state values before rendering list/cards
    // console.log("Rendering Products - isSingleVendorSelected:", isSingleVendorSelected, "singleSelectedVendorId:", singleSelectedVendorId, "activeVendorDomain:", activeVendorDomainForProductLink);
          ...
          <div className="page-products__main-wrapper">
            {filteredProducts.length > 0 ? (
              ...
              isAnyFilterActive && baseCategoryProducts.length > 0 ? (
                <div id="no-results"><h3>Δεν βρέθηκαν προϊόντα για <strong>{currentCategory?.name || 'αυτή την κατηγορία'}</strong> με αυτά τα φίλτρα.</h3><div id="no-results-suggestions"><p><strong>Προτάσεις:</strong></p><ul><li>Δες <Link to={location.pathname} onClick={(e) => { e.preventDefault(); handleResetFilters(); }}>όλα τα προϊόντα</Link>.</li><li><Link to="#" onClick={(e) => { e.preventDefault(); handleResetFilters(); }}>Αφαίρεσε κάποιο φίλτρο</Link>.</li><li>Επιστροφή στην <Link to="/">αρχική</Link>.</li></ul></div></div>
              ) : currentCategory && !currentCategory.isMain ? ( <p>Δεν υπάρχουν προϊόντα για αυτήν την κατηγορία.</p> ) : null // Show message only for leaf categories
            )}
          </div>
        </main>
      </div>
        ...
        <div className="root">
          <div data-tooltip-no-border="" data-tooltip={`Πληροφορίες: ${vendor.name} (${vendor.certification})`}>
            <div className="merchant-logo">
              <Link to={`/m/${vendor.id}/${vendor.name?.toLowerCase().replace(/\s+/g, '-') || vendor.id}`}> <img loading="lazy" src={vendor.logo} width={90} height={30} alt={`${vendor.name} logo`} /> </Link>
          ...
          <div className="information__content"><p>Προϊόντα από: <strong><Link to={`/m/${vendor.id}/${vendor.name?.toLowerCase().replace(/\s+/g, '-') || vendor.id}`}>{vendor.name}</Link></strong></p><p><Link to="#" onClick={removeThisVendorFilter}>Αφαίρεση φίλτρου</Link></p></div>
  ...
  const isLoading = !currentCategory && location.pathname.startsWith('/cat/'); // Initial loading state heuristic
  if (isLoading && defaultCategoryId === null) return <NotFound />; // Or a loading indicator
  if (!isLoading && !currentCategory) return <NotFound />; // Definite not found after trying to load
