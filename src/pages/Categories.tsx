...
const ALERT_BUTTON_THRESHOLD = 24; // Show alert button after this many products
...
const Categories: React.FC = () => {
  ...
  const sortProducts = (productsList: Product[]) => { const sorted = [...productsList]; const getPrice = (p: ProductPrice) => p.discountPrice || p.price; const getMinPrice = (prod: Product) => Math.min(...(prod.prices || []).filter(p => p.inStock).map(getPrice), Infinity); const getMaxPrice = (prod: Product) => Math.max(...(prod.prices || []).filter(p => p.inStock).map(getPrice), 0); const getStockCount = (prod: Product) => (prod.prices || []).filter(p => p.inStock).length; const getDate = (prod: Product) => new Date(prod.releaseDate || prod.dateAdded || 0).getTime(); const getRating = (prod: Product) => ({ r: prod.rating || 0, rev: prod.reviews || 0 }); switch (sortType) { case 'price-asc': sorted.sort((a, b) => getMinPrice(a) - getMinPrice(b)); break; case 'price-desc': sorted.sort((a, b) => getMaxPrice(b) - getMaxPrice(a)); break; case 'alpha-asc': sorted.sort((a, b) => (a.title || '').localeCompare(b.title || '')); break; case 'reviews-desc': sorted.sort((a, b) => (b.reviews || 0) - (a.reviews || 0)); break; case 'brand-asc': sorted.sort((a, b) => (a.brand || '').localeCompare(b.brand || '')); break; case 'merchants_desc': sorted.sort((a, b) => getStockCount(b) - getStockCount(a)); break; case 'newest-desc': sorted.sort((a, b) => getDate(b) - getDate(a)); break; case 'rating-desc': default: sorted.sort((a, b) => { const ra = getRating(a); const rb = getRating(b); return (rb.r - ra.r) || (rb.rev - ra.rev); }); break; } return sorted; };
  ...
  // *** 2. UPDATED Dynamic H1 Title Logic with Limits ***
  const dynamicPageTitle = useMemo(() => {
    ...
    // Add vendor info first
    if (selectedVendor) {
      title += ` από ${selectedVendor.name}`;
    }

    // Collect spec strings
    Object.entries(activeFilters.specs).forEach(([key, values]) => {
      if (values.length === 1) {
        specStrings.push(`${values[0]} ${key}`); // Format: Value Key
      } else if (values.length > 1) {
        specStrings.push(`${key}: ${values.join('/')}`); // Example for multi-value specs
      }
    });
    ...
    // Check character limit only if we initially took 3 specs
    if (initialSpecCount === MAX_SPECS_IN_TITLE) {
        const prefix = selectedVendor ? ' με ' : ' ';
        const potentialTitle = title + prefix + finalSpecParts.join(' & ');
        if (potentialTitle.length > DYNAMIC_TITLE_CHAR_LIMIT) {
            // If too long with 3, reduce to 2
            finalSpecParts = specStrings.slice(0, MAX_SPECS_IN_TITLE - 1);
        }
    }

    // Append the final spec parts
    if (finalSpecParts.length > 0) {
      const prefix = selectedVendor ? ' με ' : ' '; // Use 'με' only if vendor exists
      title += prefix + finalSpecParts.join(' & ');
    }

    return title;
  }, [currentCategory, selectedVendor, activeFilters.specs]);
  // *** End Dynamic H1 Title Logic ***
        ...
        <main className="page-products__main">
            ...
            <div className="page-header__title-wrapper">
              <div className="page-header__title-main">
                 ...
                 <div className="page-header__count-wrapper">
                   ...
                   {/* Minimal alert button removed from here */}
                 </div>
              </div>
              <div className="page-header__title-aside">
                {displayedBrand && displayedBrand.logo && ( <Link to={`/b/${displayedBrand.id}/${displayedBrand.slug || displayedBrand.name.toLowerCase()}.html`} title={displayedBrand.name} className="page-header__brand"><img src={displayedBrand.logo} alt={`${displayedBrand.name} logo`} height="70" loading="lazy"/></Link> )}
          ...
          <div className="page-products__main-wrapper">
            {filteredProducts.length > 0 ? (
              ...
              ) : null // Don't show anything if no filters applied and base products exist
            )}
          </div>
        </main>
      </div>
    ...
    return (
      <>
        ...
        {!showProductsInsteadOfChildren ? (
          ...
          // Render the product list structure (sidebar + main with DYNAMIC H1)
          renderProducts()
        )}
