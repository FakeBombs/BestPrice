...
const ALERT_BUTTON_THRESHOLD = 24; // Show alert button after this many products in the grid
...
const Categories: React.FC = () => {
  ...
  // *** 2. Dynamic H1 Title Logic ***
  const dynamicPageTitle = useMemo(() => {
    if (!currentCategory) return ''; // Should not happen if called within renderProducts
    ...
    Object.entries(activeFilters.specs).forEach(([key, values]) => {
      // Assuming single value selection per spec for simplicity in title
      if (values.length === 1) {
        // Simple format: "Value Key". Adjust format/prepositions as needed.
        // E.g., "8GB RAM", "128GB χωρητικότητα"
        specParts.push(`${values[0]} ${key}`);
      } else if (values.length > 1) {
        // Handle multiple values for one spec key if necessary (e.g., Colors)
        specParts.push(`${key}: ${values.join('/')}`); // Example format
      }
    });

    if (specParts.length > 0) {
      if (selectedVendor) {
        title += ` με ${specParts.join(' & ')}`; // Add "με" only if vendor was also present
      } else {
        // If no vendor, the first spec doesn't need "με" unless desired
        title += ` ${specParts.join(' & ')}`; // Adjust leading space/preposition if needed
      }
    }
  ...
  const renderProducts = () => {
    ...
    return (
      <div className="page-products">
         {/* ASIDE FILTERS */}
         {baseCategoryProducts.length > 0 && ( // Render aside only if base products exist
            ...
            <div className="page-header__title-wrapper">
              <div className="page-header__title-main">
                 ...
                 <div className="page-header__count-wrapper">
                   ...
                   {/* *** 1. REMOVED Minimal Price Alert Button *** */}
                 </div>
              </div>
              <div className="page-header__title-aside">
                {displayedBrand && displayedBrand.logo && ( <Link to={`/b/${displayedBrand.id}/${displayedBrand.slug || displayedBrand.name.toLowerCase()}.html`} title={displayedBrand.name} className="page-header__brand"><img src={displayedBrand.logo} alt={`${displayedBrand.name} logo`} height="70" loading="lazy"/></Link> )}
              ...
              <div className="p__products" data-pagination="">
                {filteredProducts.map((product, index) => (
                  <React.Fragment key={product.id}>
                    ...
                    {index === ALERT_BUTTON_THRESHOLD - 1 && (
        ...
        <div className="page-header">
          <div className="hgroup">
            <div className="page-header__title-wrapper">
              ...
              {/* Static H1 for Subcategory View */}
              <h1>{category.name}</h1>
            </div>
          </div>
        </div>
