...
import {
  ...
  Vendor,
  Brand,
  ProductPrice // Added ProductPrice for type safety
} from '@/data/mockData';
...
const Categories: React.FC = () => {
  ...
  const reconcileFilters = ( filtersFromUrl: ActiveFiltersState, currentAvailableBrands: Record<string, number>, currentAvailableSpecs: Record<string, Set<string>> ): ActiveFiltersState => {
    ...
    const reconciledSpecs = Object.entries(filtersFromUrl.specs).reduce((acc, [keyFromUrl, valsFromUrl]) => {
      ...
      if (originalKey) { const availableValuesSet = currentAvailableSpecs[originalKey]; if (availableValuesSet) { const originalValues = valsFromUrl.map(lv => Array.from(availableValuesSet).find(av => av.toLowerCase() === lv)).filter((v): v is string => !!v); if (originalValues.length > 0) acc[originalKey] = originalValues; } } return acc;
    }, {} as Record<string, string[]>);
  ...
  const handlePriceAlert = () => {
    ...
    if (currentCategory) { setPriceAlertContext({ categoryId: currentCategory.id, categoryName: currentCategory.name, filters: activeFilters }); setIsPriceAlertModalOpen(true); }
    else { toast({ title: 'Error', description: 'Cannot set alert, category context is missing.', variant: 'destructive' }); }
  };
  ...
  const renderBreadcrumbs = () => {
    ...
    if (currentCategory) {
      const ancestors: Category[] = []; let category: Category | undefined = currentCategory;
      while (category?.parentId !== null && category?.parentId !== undefined) { const parent = allCategories.find((cat) => cat.id === category?.parentId); if (parent) { ancestors.unshift(parent); category = parent; } else category = undefined; }
    ...
    return (
      <div id="trail">
        <nav className="breadcrumb">
          <ol>
            {trailItems.reduce((acc: React.ReactNode[], item, index) => { if (index > 0) acc.push(<span key={`sep-${index}`} className="trail__breadcrumb-separator">›</span>); acc.push(item); return acc; }, [])}
          </ol>
        </nav>
      </div>
    );
    ...
    return (
      <>
        <div className="page-header">
          <div className="hgroup"><div className="page-header__title-wrapper"><h1>{currentCategory.name}</h1></div></div>
        </div>
        <div className="root-category__categories">
          {subcategories.length > 0 ? (
            ...
            <p>Δεν υπάρχουν υποκατηγορίες.</p>
          )}
        </div>
  ...
  const renderAppliedFilters = () => {
    const { brands, specs, deals, certified, nearby, boxnow, instock } = activeFilters;
  ...
  // Renders the sidebar + main product listing area
  const renderProducts = () => {
    const { brands: activeBrandFilters, specs: activeSpecFilters, vendorIds: activeVendorIds, ...restActiveFilters } = activeFilters;
    const isAnyFilterActiveForChips = activeBrandFilters.length > 0 || Object.values(activeSpecFilters).some(v => v.length > 0) || Object.values(restActiveFilters).some(v => v === true); // Excludes vendorIds for chip display logic
    const isAnyFilterActiveForNoResults = isAnyFilterActiveForChips || activeVendorIds.length > 0; // Includes vendorIds for 'no results' message logic
    const isAnyFilterActiveForSidebar = isAnyFilterActiveForNoResults; // Same logic for sidebar clear button
    ...
    return (
      <div className="page-products">
          ...
          <div id="filters" role="complementary" aria-labelledby="filters-header" data-label={currentCategory.name}>
            ...
            <div className="filter-limit default-list">
              ...
              <div className="filter-container"><ol>
                <li data-filter="deals" className={`pressable ${activeFilters.deals ? 'selected' : ''}`}>
                  <Link to="#" title="Προσφορές" rel="nofollow" onClick={(e) => handleLinkFilterClick(e, handleDealsToggle)}><svg aria-hidden="true" className="icon" width={16} height={16}><use href="/dist/images/icons/icons.svg#icon-flame-16"></use></svg><span>Προσφορές</span></Link>
                </li>
                <li data-filter="certified" className={`pressable ${activeFilters.certified ? 'selected' : ''}`}>
                  <Link to="#" title="Πιστοποιημένα καταστήματα" rel="nofollow" onClick={(e) => handleLinkFilterClick(e, handleCertifiedToggle)}><svg aria-hidden="true" className="icon" width={16} height={16}><use href="/dist/images/icons/icons.svg#icon-certified-16"></use></svg><span>Πιστοποιημένα</span></Link>
                </li>
                  ...
                  <ol aria-expanded={showMoreBrands}>
                    {sortedAvailableBrandKeys.slice(0, showMoreBrands ? sortedAvailableBrandKeys.length : MAX_DISPLAY_COUNT).map((brand) => (
                      <li key={brand} className={`pressable ${activeFilters.brands.includes(brand) ? 'selected' : ''}`}>
                        <a href="#" onClick={(e) => { e.preventDefault(); handleBrandFilter(brand); }} data-c={availableBrands[brand]}>{brand}</a>
                      </li>
                    ))}
                  </ol>
                  {sortedAvailableBrandKeys.length > MAX_DISPLAY_COUNT && (
                    <div className="filters-more-prompt pressable" onClick={() => setShowMoreBrands(prev => !prev)} title={showMoreBrands ? "Λιγότεροι" : "Όλοι"}>
            ...
            {sortedAvailableSpecKeys.length > 0 && (
              sortedAvailableSpecKeys.map((specKey) => {
                const specValuesSet = availableSpecs[specKey]; if (!specValuesSet || specValuesSet.size === 0) return null; const specValuesArray = Array.from(specValuesSet).sort(); const isExpanded = showMoreSpecs[specKey] || false;
                return (
                      ...
                      <ol aria-expanded={isExpanded}>
                        {specValuesArray.slice(0, isExpanded ? specValuesArray.length : MAX_DISPLAY_COUNT).map((specValue) => (
                          <li key={specValue} className={`pressable ${activeFilters.specs[specKey]?.includes(specValue) ? 'selected' : ''}`}>
                            <a href="#" onClick={(e) => {e.preventDefault(); handleSpecFilter(specKey, specValue); }}><span>{specValue}</span></a>
                          </li>
                        ))}
                      </ol>
                      {specValuesArray.length > MAX_DISPLAY_COUNT && (
                        <div className="filters-more-prompt pressable" onClick={() => setShowMoreSpecs(prev => ({...prev, [specKey]: !prev[specKey]}))} title={isExpanded ? `Λιγότερες ${specKey}` : `Όλες οι ${specKey}`}>
                           <svg aria-hidden="true" className="icon" width={10} height={10} viewBox="0 0 10 10"><path fillRule="evenodd" d={isExpanded ? "M9.5 6H0.5C0.224 6 0 5.776 0 5.5V4.5C0 4.224 0.224 4 0.5 4H9.5C9.776 4 10 4.224 10 4.5V5.5C10 5.776 9.776 6 9.5 6Z" : "M6 4V0.5C6 0.224 5.776 0 5.5 0H4.5C4.224 0 4 0.224 4 0.5V4H0.5C0.224 4 0 4.224 0 4.5V5.5C0 5.776 0.224 6 0.5 6H4V9.5C4 9.776 4.224 10 4.5 10H5.5C5.776 10 6 9.776 6 9.5V6H9.5C9.776 6 10 5.776 10 5.5V4.5C10 4.224 9.776 4 9.5 4H6Z"} /></svg>
                          {isExpanded ? "Λιγότερα" : "Όλα"}
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <main className="page-products__main">
          {/* Header Section for Product List */}
          <header className="page-header">
            <div className="page-header__title-wrapper">
              <div className="page-header__title-main">
                {/* Count and Alert Button */}
                <div className="page-header__count-wrapper">
                  <div className="page-header__count">{filteredProducts.length} {filteredProducts.length === 1 ? 'προϊόν' : 'προϊόντα'}</div>
                  {(isAnyFilterActiveForNoResults || baseCategoryProducts.length > 0) && filteredProducts.length > 0 && currentCategory && (
              ...
              <div className="page-header__title-aside">
                {displayedBrand && displayedBrand.logo && (
                  <Link to={`/b/${displayedBrand.id}/${displayedBrand.slug || displayedBrand.name.toLowerCase()}.html`} title={displayedBrand.name} className="page-header__brand">
                    <img src={displayedBrand.logo} alt={`${displayedBrand.name} logo`} height="70" loading="lazy"/>
                  </Link>
                )}
              </div>
            </div>

            {/* Applied Filters (excluding vendor chip) */}
            ...
            {filteredProducts.length > 0 && (
              <div className="page-header__sorting">
                <div className="tabs"><div className="tabs-wrapper"><nav>
                  ...
                  <a href="#" data-type="reviews-desc" rel="nofollow" className={sortType === 'reviews-desc' ? 'current' : ''} onClick={(e) => { e.preventDefault(); handleSortChange('reviews-desc'); }}><div className="tabs__content">Αξιολογήσεις</div></a>
                  {shouldShowBrandSort && ( <a href="#" data-type="brand-asc" rel="nofollow" className={sortType === 'brand-asc' ? 'current' : ''} onClick={(e) => { e.preventDefault(); handleSortChange('brand-asc'); }}><div className="tabs__content">Κατασκευαστή</div></a> )}
                  <a href="#" data-type="merchants_desc" rel="nofollow" className={sortType === 'merchants_desc' ? 'current' : ''} onClick={(e) => { e.preventDefault(); handleSortChange('merchants_desc'); }}><div className="tabs__content">Καταστήματα</div></a>
                </nav></div></div>
              </div>
            )}
          </header>

          {/* Product Grid / No Results Wrapper */}
          <div className="page-products__main-wrapper">
            {filteredProducts.length > 0 ? (
              <div className="p__products" data-pagination="">
                {filteredProducts.map((product) => (
                  <ProductCard key={product.id} product={product} activeVendorFilterDomain={activeVendorDomainForProductLink}/>
                ))}
              </div>
            ) : (
              isAnyFilterActiveForNoResults && baseCategoryProducts.length > 0 ? (
                <div id="no-results">
                  <h3>Δεν βρέθηκαν προϊόντα για <strong>{currentCategory?.name || 'αυτή την κατηγορία'}</strong> με αυτά τα φίλτρα.</h3>
                  <div id="no-results-suggestions">
                    <p><strong>Προτάσεις:</strong></p>
                    <ul>
                      <li>Δες <Link to={location.pathname} onClick={(e) => { e.preventDefault(); handleResetFilters(); }}>όλα τα προϊόντα</Link>.</li>
                      <li><Link to="#" onClick={(e) => { e.preventDefault(); handleResetFilters(); }}>Αφαίρεσε κάποιο φίλτρο</Link>.</li>
                      <li>Επιστροφή στην <Link to="/">αρχική</Link>.</li>
                    </ul>
                  </div>
                </div>
              ) : currentCategory && !currentCategory.isMain ? ( <p>Δεν υπάρχουν προϊόντα για αυτήν την κατηγορία.</p> ) : null
            )}
          </div>
        </main>
      </div>
   ...
   // Renders the content for subcategories (either more subcats or the product list)
   const renderSubcategoryContent = () => {
    if (!currentCategory || currentCategory.isMain) return null; // Only for non-main categories
    ...
    return (
      <>
        {/* Subcategory Header (Back button + Title) */}
        <div className="page-header">
        ...
        {childCategories.length > 0 ? (
          ...
          // Render the actual product listing (sidebar + main)
          renderProducts()
        )}

        <div className="sections"></div>

        {/* Price Alert section only for leaf categories where products exist */}
        {childCategories.length === 0 && baseCategoryProducts.length > 0 && currentCategory && (
   ...
   // Renders the top banner when a single vendor is selected
  const renderMerchantInformation = () => {
    if (!selectedVendor) return null;
    const vendor = selectedVendor;
    const removeThisVendorFilter = (e: React.MouseEvent) => { e.preventDefault(); handleMultiVendorToggle(vendor); }; // Clears vendor filter
        ...
        <div className="root">
          <div data-tooltip-no-border="" data-tooltip={`Πληροφορίες: ${vendor.name} (${vendor.certification})`}>
            <div className="merchant-logo">
              <Link to={`/m/${vendor.id}/${vendor.name?.toLowerCase().replace(/\s+/g, '-') || vendor.id}`}>
          ...
          <div className="information__content">
            <p>Προϊόντα από: <strong><Link to={`/m/${vendor.id}/${vendor.name?.toLowerCase().replace(/\s+/g, '-') || vendor.id}`}>{vendor.name}</Link></strong></p>
            <p><Link to="#" onClick={removeThisVendorFilter}>Αφαίρεση φίλτρου</Link></p>
          </div>
          <span>
            <svg aria-hidden="true" className="icon information__close pressable" width={12} height={12} onClick={removeThisVendorFilter}><use href="/dist/images/icons/icons.svg#icon-x-12"></use></svg>
          </span>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      <div className="root__wrapper root-category__root">
        <div className="root">
          {renderBreadcrumbs()}
          {/* Render main category view OR subcategory/product view */}
          {currentCategory?.isMain ? renderMainCategories() : renderSubcategoryContent()}
          {/* Price Alert Modal (conditionally rendered) */}
