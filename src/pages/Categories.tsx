...
const Categories: React.FC = () => {
  ...
  // --- Slider/Section Rendering Functions (Defined first as they are called by Main/Sub) ---
  ...
  const renderPopularCategoriesSection = (categoriesToConsider: Category[]) => {
      if (!categoriesToConsider || categoriesToConsider.length === 0) return null;
      // Sort the provided categories by popularity (descending product count)
      const popularToShow = [...categoriesToConsider]
  ...
  const renderSubcategories = (category: Category) => {
    ...
    const childCategories = allCategoriesList.filter(cat => cat.parentId === category.id); // Use combined list
        ...
        {!showProductsInsteadOfChildren ? (
          <>
            <div className="root-category__categories">
              {childCategories.map((subCat) => (<div key={subCat.id} className="root-category__category"><Link to={`/cat/${subCat.id}/${subCat.slug}`} className="root-category__cover"><img src={subCat.image || '/dist/images/cat/placeholder.webp'} alt={subCat.name} title={subCat.name} loading="lazy" width="200" height="150"/></Link><h3 className="root-category__category-title"><Link to={`/cat/${subCat.id}/${subCat.slug}`}>{subCat.name}</Link></h3>{/* Footer removed from here */}</div>))}
            </div>
