...
const CategoryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Find the correct category through slugs
    if (subCatSlug) {
    ...
    // Set the current category to either the found category or the main category
      ...
      <>
        ...
        <div className="root-category__categories">
          {subcategories.length > 0 ? (
            ...
            renderProducts() // Show products if no subcategories exist
          )}
        </div>
      </>
    );
      ...
      <main className="page-products__main">
        <header className="page-header">
          ...
          <div className="page-header__sorting">
            <nav>
              <a className={sortType === 'rating-desc' ? 'current' : ''} onClick={() => setSortType('rating-desc')}>Δημοφιλέστερα</a>
              <a className={sortType === 'price-asc' ? 'current' : ''} onClick={() => setSortType('price-asc')}>Φθηνότερα</a>
              <a className={sortType === 'price-desc' ? 'current' : ''} onClick={() => setSortType('price-desc')}>Ακριβότερα</a>
              <a className={sortType === 'merchants_desc' ? 'current' : ''} onClick={() => setSortType('merchants_desc')}>Αριθμός καταστημάτων</a>
            </nav>
          </div>
        </header>
    ...
    <div className="root__wrapper root-category__root">
      <div className="root">
        ...
        {currentCategory 
          ...
          : renderProducts()} {/* Render products if no current category is found */}
      </div>
    </div>
  );
};
