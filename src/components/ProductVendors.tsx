...
// Helper to clean domain name (needed for popup link if used)
...
export const VendorPriceCard = ({ priceInfo, product, openPopup }: VendorPriceCardProps) => { // Use NAMED export
  // Find vendor efficiently using useMemo and precomputed map idea (or direct find for simplicity)
  ...
  return (
    // Using Card component structure as per original VendorPage example
    <Card className={`prices__card ${!priceInfo.inStock ? 'is-unavailable' : ''}`}>
      <CardContent className="p-0"> {/* Assuming no padding needed directly on CardContent */}
        <div className="prices__group" data-id={vendor.id} data-price={Math.round(displayPrice * 100)} data-mid={vendor.id} data-domain={vendor.url} data-mrating={vendor.rating?.toFixed(4) || '0'}>
          ...
          {(vendor.paymentMethods || vendor.certification) && (
          <div className="prices__footer">
            <div className="prices__footer-items">
              ...
              {vendor.paymentMethods?.includes(PaymentMethod.PickupVia) && (
                  <div className="prices__footer-item" data-tooltip-left="" data-tooltip-no-border="" data-tooltip="Δυνατότητα παραλαβής δέματος μέσω των αυτόματων μηχανημάτων (lockers) της Box Now">
                    <div className="prices__boxnow-logo"> <img src="/images/logos/BoxNow/logo.svg" alt="Box Now logo" width={27} height={20} /> </div>
                  </div>
              )}
               {/* Add other payment method icons similarly if needed */}
            </div>
          </div>
          )}
        </div>
