...
// Language Modal Component (Restored with Facebook-like structure)
...
// This list should be outside the component to be a stable reference
const ALL_AVAILABLE_LANGUAGES: LanguageOption[] = [
  ...
  { code: 'tr', name: 'Türkçe', englishName: 'Turkish', regionKey: 'languageCategoryAfrica' }, // Also consider putting in Europe
  { code: 'sw', name: 'Kiswahili', englishName: 'Swahili', regionKey: 'languageCategoryAfrica' },
  // You would expand this list significantly
];

// This list should also be outside the component
...
const LanguageModal: React.FC<LanguageModalProps> = ({ isOpen, onClose }) => {
  ...
  // Use a stable default for sorting if language is not yet loaded
  ...
  const handleLanguageChange = (langCode: string) => {
    ...
    if (['en', 'el', 'es', 'fr', 'de'].includes(simpleLangCode)) {
        ...
        if (['en', 'el', 'es', 'fr', 'de'].includes(simpleLangCode)){ // Try with base if valid
            setContextLanguage(simpleLangCode);
        }
    }
    onClose();
  ...
  // Memoize suggestedLangsToDisplay because ALL_AVAILABLE_LANGUAGES is constant
  ...
  }, []); // ALL_AVAILABLE_LANGUAGES is defined outside, so it's stable.
  ...
  const languagesToDisplay = useMemo(() => {
    ...
    if (selectedRegion === "suggested") {
        ...
        // Filter creates a new array, which is fine for the input to sort
        listToFilter = ALL_AVAILABLE_LANGUAGES.filter(lang => lang.regionKey === selectedRegion);
    }
    // Create a new array before sorting to avoid mutating the source (suggestedLangsToDisplay or the filtered list)
        ...
        <div className="flex flex-1 overflow-hidden">
          <aside className="w-1/3 border-r border-border overflow-y-auto p-2 space-y-1 bg-muted/20">
            {LANGUAGE_REGIONS_FOR_MODAL.map(region => (
              ...
              >
                {/* Provide a default for region.nameKey's second arg if t() expects one */}
...
const Footer: React.FC = () => {
    const { t, language } = useTranslation(); // language is used for date formatting
